...
| `limit`      | `integer`  | максимальное количество записей в каждом потоке                         |
| `direction`  | `string`   | порядок сортировки значений (`forward` или `backward`)                   |
| `regexp`     | `string`   | дополнительный фильтр по регулярному выражению для значений             |
