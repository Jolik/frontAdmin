...
type
  ...
  TParentFieldSetForm = class(TUniForm)
...
function ParentForm: TParentFieldSetForm;
...
function TParentFieldSetForm.NewCallback(const AID: string;AEntity: TFieldSet):boolean;
var
  ReqNew: TReqNew;
...
procedure TParentFieldSetForm.OnCreate;
begin
  //
end;

function TParentFieldSetForm.UpdateCallback(const AID: string; AEntity: TFieldSet):boolean;
var
  ReqUpd: TReqUpdate;
  JsonRes: TJSONResponse;
  LEditedId: string;
...
function TParentFieldSetForm.GetCurrentEntityId: string;
begin
  Result := '';
...
procedure TParentFieldSetForm.UniFormCreate(Sender: TObject);
begin
  // создаем брокера
...
procedure TParentFieldSetForm.UniFormDestroy(Sender: TObject);
begin
  FreeAndNil(FRestBroker);
  // нужно удалять или нет? FreeAndNil(EditForm);
end;

procedure TParentFieldSetForm.PrepareEditForm(isEditMode: boolean);
begin
  FEditForm := CreateEditForm();
...
function ParentForm: TParentFieldSetForm;
begin
  Result := TParentFieldSetForm(UniMainModule.GetFormInstance(TParentFieldSetForm));
end;

{ Default implementations for REST factories }

function TParentFieldSetForm.CreateRestBroker: TRestFieldSetBroker;
begin
  Result := nil;
end;
