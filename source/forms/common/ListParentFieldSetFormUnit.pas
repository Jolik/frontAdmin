unit ListParentFieldSetFormUnit;
...
uses
  ...
  EntityUnit, ParentFieldSetFormUnit, ParentEditFormUnit, uniLabel, StrUtils,
  RestBrokerBaseUnit, BaseRequests, BaseResponses, HttpClientUnit;

type
  ///  базовая форма с таблицей-списком
  TListParentFieldSetForm = class(TParentFieldSetForm)
...
function ListParentForm: TListParentFieldSetForm;
...
{  TListParentFieldSetForm  }

procedure TListParentFieldSetForm.btnUpdateClick(Sender: TObject);
var
  LFieldSet: TFieldSet;
  //LId : string;
...
procedure TListParentFieldSetForm.dbgEntitySelectionChange(Sender: TObject);
var
  LId     : string;
  req :  TReqInfo;
...
procedure TListParentFieldSetForm.OnInfoUpdated(AFieldSet: TFieldSet);
var
   DT      : string;
begin
...
procedure TListParentFieldSetForm.btnNewClick(Sender: TObject);
var
  LFieldSet: TFieldSet;
...
procedure TListParentFieldSetForm.btnRefreshClick(Sender: TObject);
var
  LId : string;
...
procedure TListParentFieldSetForm.btnRemoveClick(Sender: TObject);
var
  LId : string;
...
procedure TListParentFieldSetForm.Refresh(const AId: String = '');
var
    ...
    try
      ...
      if FDMemTableEntity.IsEmpty then
        ...
        on E: EIdHTTPProtocolException do begin
          ...
          Log('TListParentFieldSetForm.Refresh ' + e.Message+' '+E.ErrorMessage, lrtError);
        end;
        on E: Exception do begin
          MessageDlg('Ошибка получения спика : ' + E.Message, mtWarning, [mbOK], nil);
          Log('TListParentFieldSetForm.Refresh ' + e.Message, lrtError);
        end;
    end;      
  finally
...
function TListParentFieldSetForm.UpdateCallback(const AID: string; AFieldSet: TFieldSet):boolean;
begin
  inherited;
...
procedure TListParentFieldSetForm.UniFormCreate(Sender: TObject);
begin
  inherited;
  Refresh();
end;
...
function ListParentForm: TListParentFieldSetForm;
begin
  Result := TListParentFieldSetForm(UniMainModule.GetFormInstance(TListParentFieldSetForm));
